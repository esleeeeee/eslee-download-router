...
- 선택창의 **앞으로 10분간 이 경로로 저장** — 현재 파일과 이후 같은 사이트 규칙의 새 다운로드를 선택한 폴더로 자동 저장. 10분 후 다시 선택창이 뜨며, 다른 규칙·기존 대기 파일에는 적용하지 않습니다. 남은 시간은 재시작 후에도 유지되고 파일명은 재사용하지 않습니다.
- 중간에 해제하려면 **대시보드 → 10분 자동 저장 → 취소**를 누르세요. 해당 규칙의 이후 다운로드부터 다시 위치를 물으며, 이미 지정된 파일은 영향을 받지 않습니다.
