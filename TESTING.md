...
- 관련 통합 테스트 3건 통과: 취소·중복 취소·재시작 후 해제 유지 및 이미 지정된 파일의 완료, 다른 규칙 설정 보존과 잘못된 규칙 ID 거부, 다운로드 이벤트 없이도 만료 항목 목록 제외.
- Debug 빌드 경고·오류 0. 사용자 요청에 따라 추가 UI 검증 없이 Release 빌드 및 설치 파일을 배포합니다.

### 1.3.0: 10분 경로 재사용

- 통합 테스트 5건 추가: 재시작 후 유지·만료 경계·만료 뒤 완료, 규칙별 격리·취소·Automatic, 규칙 변경/폴더 소실 fallback, 일반 선택·Skipped 재처리 방지.
- .NET 204/204, Extension 28/28 통과. 설치본에서 버튼 표시를 확인했고 격리 Native Messaging 명령으로 같은 규칙의 자동 경로 적용과 다른 규칙 분리를 확인했습니다.
- UI 입력 자동화는 도구 오류로 완료하지 못했습니다. 이후 사용자가 정상 동작을 확인하고 추가 검증 없이 릴리즈하도록 지시했습니다.
- 업그레이드 완료 및 기존 규칙·이력 ID·이벤트·설정 보존 확인. 설치 중 브라우저 연결로 앱이 재실행되어 한 차례 파일 잠금이 발생했으며, Native Host 실행 경로를 잠시 보류하고 재설치해 해결했습니다.
