...
- 대시보드 상단에 적용 중인 10분 자동 저장의 규칙·경로·종료 시각과 취소 버튼을 추가했습니다. 취소는 규칙별로 영구 반영되며 이후 다운로드부터 다시 위치를 묻습니다. 이미 경로가 지정된 파일은 그대로 처리합니다.

## [1.3.0] - 2026-09-22

- 개별 저장 위치 선택창에 `앞으로 10분간 이 경로로 저장` 버튼 추가. 현재 파일과 이후 같은 규칙으로 시작되는 다운로드에 적용하며, 파일명은 재사용하지 않습니다.
- 만료 시 다시 선택창을 표시합니다. 남은 유효 시간은 Agent 재시작 후에도 유지되며, 규칙 변경이나 대상 폴더 소실 시 일반 선택으로 돌아갑니다. 기존 대기 항목과 Automatic은 유지합니다.

## [1.2.0] - 2026-09-20

### 다운로드 위치·이름 선택

- 개별 다운로드 선택창에 기본 파일명이 채워진 이름 입력란(확장자 포함)을 추가했습니다. 변경하지 않으면 브라우저의 최종 파일명을 사용합니다.
- 수동 저장 위치 선택에서 상위 폴더와 형제 폴더로 이동할 수 있습니다. 규칙의 기본 폴더 및 Automatic 경로 제한은 유지합니다.
- 선택 경로·이름을 Job에 저장하고 완료 시 적용합니다. 예약 이름·경로 입력은 거부하고 같은 이름의 기존 파일은 덮어쓰지 않습니다. DB migration 6은 nullable 열 두 개만 추가합니다.

## [1.1.4] - 2026-08-10

GitHub Release로 게시한 버전입니다.
